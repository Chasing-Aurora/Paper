...
##### [新版本 就是双向同步](https://zotero.yuque.com/staff-gkhviy/better-notes/aid2c3)
- 同步，需要better notes设置导出为maarkdwon-将链接的笔记分别导出-勾选为每个笔记设置自动同步，才可以！！！
- 同步检查有多种可能触发：  
	- Zotero笔记产生编辑时，添加后台任务自动对编辑的笔记进行同步检查；  
	 - 用户点击同步窗口中的**同步按钮**，对当前笔记或全部设置同步的笔记进行同步检查；  
	- Zotero主窗口存在于**前台且激活**时（即，当前选中的窗口是Zotero），定期对全部设置同步的笔记进行同步检查。  
...
- ==平时阅读 主要还是 在 **zotero** 上边进行的，还能**本地 TTS 阅读的**==
	...
	- ==每一段的 "重点高亮"、解释每一个公式、问全文问题、解释某一个的参考文献： 可以结合  Scholaread==
- ==如果是高引顶会 论文，去yotube搜 **精度论文** 的视频==
	- 然后 关键的地方 自己对照pdf  去
	- 这样子  比自己 去琢磨很好的！而且是比 自己更快的，因为总归来说不是母语的
	- ==前期顶会论文还是学习别人的视频，之后再去 自己实际看==
	- 因为能学到 别人阅读 过程中的思想
- 区分 精度和略读
	- 略读 要看 intro、method、conclusion
	- 精度 就要加上 experiment
	- 自己 参考这篇做实验 加上 ablation 

- 如果是 **ArXiv 或者 顶会venues**  的，去https://chatpaper.com/zh-CN/chatpaper，这里有现成的 翻译和解释呢
	- arXiv ID 的格式通常为：_YYMM.NNNNN_，详情页链接：https://arxiv.org/abs/id PDF 文件链接：https://arxiv.org/pdf/id
...
- PDF翻译：
	- 先使用 购买的 Scholaread，AI 翻译每天 10w 字 + AI 排版每天 500 页
	- [Doc2X，也有一些免费签到额度，每月可领取 250 页 额度](https://doc2x.noedgeai.com/?invite_code=ZHDAPC)
		- Doubao-1.5-Lite-32k
		- deepseek-v4-flash
